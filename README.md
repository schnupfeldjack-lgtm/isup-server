...
项目内置了一个轻量联调页面（只在本机调用 `/api/v1/**`，无后端依赖）：

```
http://127.0.0.1:8080/debug.html
```

页面可填 API Base 与 X-API-Key，直接在线发预览 / 回放 / 下载请求，并用 hls.js 播放 m3u8。
**仅供内网联调使用**，API Key 会明文存在浏览器 localStorage 中，请勿部署到公网。
...
```
http://<服务器>:8080/debug.html
```

一个**单文件静态页**（`src/main/resources/static/debug.html`），随 jar 一起打包，无前端工程、无构建步骤、无后端依赖。用于本地联调验证「设备注册 / 预览 / 回放 / 下载」是否打通。

### 能做什么

| 功能 | 说明 |
|------|------|
| 健康检查 | 显示 SDK 是否就绪、在线设备数、活跃会话数、`public-ip`、三个端口、运行时长 |
| 在线设备列表 | 点击任意一行把 `deviceId` 填入下方三个表单 |
| 实时预览 | 选通道 / 码流 / 链路，开预览后自动轮询到 `RUNNING` 再拉 HLS |
| 按时间段回放 | 支持「最近 10 分钟 / 1 小时 / 24 小时」一键填充（按**本机**时间，注意与录像机时间是否一致） |
| 录像下载 | 创建任务后自动轮询状态，完成后可保存 MP4（走 fetch + blob，因为下载接口也要 `X-API-Key`） |
| 会话管理 | 展示本页发起的会话，可单独停止 |
| 请求日志 | 显示每次请求的方法、路径、状态码与响应体，排查直接看这里 |

### 注意事项

- **hls.js 走 CDN**（jsdelivr）。服务器无外网时，Chrome / Edge 无法播放 m3u8，改用 Safari 或用 `ffplay` 拉 `index.m3u8`
- 页面在顶部填的 `X-API-Key` 会明文存到浏览器 `localStorage`
- `/media/**` **不鉴权**（HLS 地址要裸着给播放器），所以这个页面和 8080 端口一样，**只能内网访问**
- HLS 切片要等设备推流后才有，页面已经做了「轮询到 RUNNING 再拉流」，不用手动等 2~5 秒

> 这是联调工具，不是产品页面。生产环境如果不需要，删掉 `src/main/resources/static/debug.html` 即可，不影响任何其他功能。
